...
public class HueServiceImpl implements HueService {
	...
	public HueServiceImpl(BridgeRepository bridgeRepository) {
		...
		this.bridges = Collections.synchronizedList(new ArrayList<>());
	}
	...
	public void updateLamp(LampWithHueUniqueId lamp, ScenarioConfigDTO config) {
		if (config != null) {
			for (PHBridge bridge : bridges) {
				...
				bridge.doHTTPGet(url, jsonResponse -> {
	...
	private void updateLightStateColor(PHLightState lightState, String lightModelNumber, String colorHex) {
		Color color = colorHex == null ? Color.WHITE : Color.decode(colorHex); // AWT ist auch OK
	...
	public void testScenario(LampTestDTO lamp) {
		for (LampNameDTO l : nullSafe(lamp.getLamps())) {
	...
	public void pulseOnce(String hueUniqueId) {
		updateLamp(() -> hueUniqueId, new ScenarioConfigDTO(-1, null, true, true, true, "#FFF", false, null, false, 0));
	}
	...
	public boolean ipAreEqual(String ip1, String ip2) {
		return removePortAndToLowerCase(ip1).equals(removePortAndToLowerCase(ip2));
	}
